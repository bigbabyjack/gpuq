...
async def test_submit_propagates_exit_code(daemon):
    ...
    terminal = [e for e in events
                if isinstance(e, p.StateEvent) and e.exit_code is not None]
...
async def test_jobs_serialized(daemon):
    ...
    times1 = [e.line.strip() for e in e1
              if isinstance(e, p.LogEvent) and e.stream == "stdout"]
    times2 = [e.line.strip() for e in e2
              if isinstance(e, p.LogEvent) and e.stream == "stdout"]
...
@pytest.mark.asyncio
async def test_show_returns_job(daemon):
...
@pytest.mark.asyncio
async def test_cancel_queued_job(daemon):
    """Submit a long-running job, then a second job, then cancel the second while queued."""
    # Block the queue with a long-running first submit (in background).
    blocker_events: list[p.Event] = []

    async def blocker():
        events = await _send_recv(p.Submit(
            cmd=["bash", "-c", "sleep 0.5"], cwd="/tmp", env={},
        ))
    ...
    # Second submit will be queued behind blocker.
    reader, writer = await asyncio.open_unix_connection(str(paths.socket_path()))
    writer.write(p.encode_request(p.Submit(
        cmd=["true"], cwd="/tmp", env={},
    )).encode())
    await writer.drain()
    ...
    # Submit's stream should see the cancelled state.
    saw_cancelled = False
